...
. It uses files whloginich stores the information of legal username and password pair to check whether the username and password key in by users are valid.
...
**Pros:** Faster to implement. Not changing the structure of code. +
...
=== Use case: Email Intents

*MSS*

1. User clicks on email button.
2. AddressBook opens up the Gmail webpage.
3. The email address of the person to be contacted is already filled in.
4. User types in the message and sends the email.
+
Use case ends.
